...
- **Поведение:**: при создании платежа формируется запрос к API YooKassa с суммой `order.total_price`; сервис возвращает `confirmation_url`, на который пользователь перенаправляется (в коде `return_url` для подтверждения указан как `https://t.me/forgot_vpn_bot`).
