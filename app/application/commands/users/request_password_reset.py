...
@dataclass(frozen=True)
class RequestPasswordResetCommandHandler(BaseCommandHandler[RequestPasswordResetCommand, None]):
    ...
    async def handle(self, command: RequestPasswordResetCommand) -> None:
        ...
        await self.mail_service.send_reset_code(user.email, code, valid_minutes=15)
